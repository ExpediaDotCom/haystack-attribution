...
/**
  * Abstract class which can be implemented to get timer and exception handling inherited
  *
  * @param endpointName endpoint to listen
  */
abstract class BaseResource(endpointName: String) extends HttpServlet with MetricsSupport {
  private val LOGGER = LoggerFactory.getLogger(classOf[BaseResource])
